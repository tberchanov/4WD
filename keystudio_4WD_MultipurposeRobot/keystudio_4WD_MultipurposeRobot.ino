...
void blinkLed() {
  if (ledCounter % 100000 == 0) {
    digitalWrite(ledPin, showLed ? HIGH : LOW);
    showLed = !showLed;
    ledCounter = 0;
  }
  ledCounter += 1;
...
void showHelloDance() {
  if (doHelloDance) {
    ...
    delay(500);
    // ISSUE For some reason servo 65 is physically equal to 90, when IR is connected in the program.
...
void IIC_send(unsigned char send_data) {
  for(char i = 0;i < 8;i++) { //Every character has 8 bits
    ...
    //pull down the SCL_Pin to change the signal of SDA
    delayMicroseconds(3);
    // 1 or 0 of byte is used to set high and low level of SDA_Pin
    if(send_data & 0x01) {
      digitalWrite(SDA_Pin,HIGH);
    } else {
      digitalWrite(SDA_Pin,LOW);
    ...
    delayMicroseconds(3);
    digitalWrite(SCL_Pin,HIGH);
    //pull up the SCL_Pin to stop transmitting data
    delayMicroseconds(3);
